                            ...
                            <div class="modal-body">
                                <div class="row">
                                    ...
                                    <div class="col-md-5">
                                        ...
                                        <div class="form-group">
                                            <label for="jumlah" class="form-label">{{__("incoming quantity")}}<span class="text-danger">*</span></label>
...
<script>
    ...
    $(document).ready(function(){
        ...
        $("#modal-button").on("click",function(){
            ...
            $('#simpan').text("{{__('save')}}").data('mode','create');
        });
    });
    ...
    $(document).on("click",".ubah",function(){
        $("#modal-button").click();
        $("#simpan").text("{{__('update')}}").data('mode','edit');
    ...
    // HAPUS data (dipakai kalau masih ada tombol .hapus di tempat lain)
    ...
    $(document).on("click",".verify",function(){
        ...
        Swal.fire({
            ...
            text: "{{__('verify this transaction')}}",
            icon: "question",
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url:"{{route('transaksi.masuk.verify')}}",
                    type:"POST",
                    data:{
                        id:id,
                        "_token":"{{csrf_token()}}"
    ...
    // VALIDASI (ADMIN) - tombol validasi hilang setelah sukses
    $(document).on("click",".validasi",function () {
        ...
        Swal.fire({
            title: "{{__('you are sure')}} ?",
            text: "{{__('validate this transaction')}}",
            icon: "question",
            showCancelButton: true,
            confirmButtonText: "{{__('yes, validate')}}",
            cancelButtonText: "{{__('cancel')}}",
        }).then((result) => {
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{route('transaksi.masuk.validate')}}",
                    type: "POST",
                    data: {
                        id: id,
                        _token: "{{ csrf_token() }}"
    ...
    $(document).on("click",".tolak",function(){
        ...
        swalWithBootstrapButtons.fire({
            title: "{{__('are you sure')}} ?",
            text: "{{__('this data will be rejected')}}",
            icon: "warning",
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url:"{{route('transaksi.masuk.reject')}}",
                    type:"POST",
                    data:{
                        id:id,
                        "_token":"{{csrf_token()}}"
