...
class ReportStockController extends Controller
{
    ...
    public function grafik(Request $request): JsonResponse
    {
        ...
        $goodsInThisMonth = GoodsIn::where('status', 'validated')
            ->whereMonth('date_received', $currentMonth)
            ->whereYear('date_received', $currentYear)
            ->sum('quantity');

        $goodsOutThisMonth = GoodsOut::where('status', 'validated')
            ->whereMonth('date_out', $currentMonth)
